...
def plot_scatter_error_vs_n_k(csv_file, imgs_dir):
    """
    ...
    for algo, col in heuristics.items():
        ...
        plt.xlabel("N (tamanho da instância)")
        plt.ylabel("Erro (%)")
        ...
        plt.title(f"{algo} — Erro vs K (corr={corr:.3f})")
        plt.xlabel("K (número de partições)")
        plt.ylabel("Erro (%)")
        plt.grid(True, linestyle="--", alpha=0.4)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"{algo}_error_vs_K_scatter.png"), dpi=150)
        plt.close()
